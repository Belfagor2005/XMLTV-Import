...
def find_channel_reference(channel_id):
    """Cerca reference per channel ID EPG - Versione semplificata per EPGImport"""
    try:
        print(f"[XMLTVConverter] Searching reference for: {channel_id}", file=log)
        
        # Pulizia base del channel ID
        clean_id = channel_id
        clean_id = clean_id.replace('(', ' ').replace(')', ' ')
        clean_id = clean_id.replace('..', '.').replace('.-.', '.')
        clean_id = ' '.join(clean_id.split())
        
        # Crea termini di ricerca
        search_terms = [clean_id]
        if ' ' in clean_id:
            search_terms.append(clean_id.replace(' ', '.'))
            search_terms.append(clean_id.replace(' ', '_'))
            search_terms.append(clean_id.split(' ')[0])
        
        # Filtra termini validi
        search_terms = [term for term in search_terms if term and len(term) > 2]
        
        print(f"[XMLTVConverter] Search terms: {search_terms}", file=log)

        # Cerca nel file lamedb
        lamedb_path = "/etc/enigma2/lamedb"
        if exists(lamedb_path):
            with open(lamedb_path, 'r', errors='ignore') as f:
                content = f.read()
                    ...
                    if term_lower in content.lower():
                        ...
                        for i, line in enumerate(lines):
                            if term_lower in line.lower():
                                if i > 0 and lines[i - 1].startswith('1:0:'):
                                    ref = lines[i - 1].strip()
                                    print(f"[XMLTVConverter] Found: {term} -> {ref}", file=log)
                                    return ref
                                if i + 1 < len(lines) and lines[i + 1].startswith('1:0:'):
                                    ref = lines[i + 1].strip()
                                    print(f"[XMLTVConverter] Found: {term} -> {ref}", file=log)
                                    return ref
        
        print(f"[XMLTVConverter] Not found: {channel_id}", file=log)
        return None
        
    except Exception as e:
        print(f"[XMLTVConverter] Error searching for {channel_id}: {e}", file=log)
        return None
...
def save_unknown_channel(channel_id):
    ...
    try:
        print(f"[XMLTVConverter] Saving unknown channel: {channel_id}", file=log)
        ...
        with open(UNKNOWN_CHANNELS_FILE, "a") as f:
            ...
            if channel_reference:
                ...
                print(f"[XMLTVConverter] Auto-found: {channel_id} -> {channel_reference}", file=log)
            else:
                f.write(f"{timestamp}|{channel_id}||NOT_FOUND\n")
                print(f"[XMLTVConverter] Not found: {channel_id}", file=log)
                
    except Exception as e:
        print(f"[XMLTVConverter] Error saving {channel_id}: {e}", file=log)
    ...
    def enumFile(self, fileobj):
        ...
        for elem in enumerateProgrammes(fileobj):
            ...
            if channel not in self.channels:
                if channel not in unknown_channels:
                    print(f"[XMLTVConverter] Unknown EPG ID: {channel}", file=log)
                    unknown_channels.add(channel)
                    save_unknown_channel(channel)
                yield None
                continue

            # SE ESISTE: processa TUTTI i reference per questo ID EPG
